...
describe("POST /auth/login", function () {
  ...
  // TEST BUG #1
  test("username/password to log in that is admin", async function () {
    const response = await request(app).post("/auth/login").send({
      username: "u3",
      password: "pwd3",
    });
...
describe("GET /users", function () {
  ...
  // TEST BUG #5
  test("should deny access when signiture is wrong on JWT", async function () {
    const response = await request(app)
      .get("/users")
      .send({ _token: `${tokens.u1}RUINSIGNITURE` });
    expect(response.statusCode).toBe(401);
  });
...
describe("GET /users/[username]", function () {
  ...
  test("returns 404 for no user found", async function () {
    const response = await request(app)
      .get("/users/notuser")
      .send({ _token: tokens.u1 });
...
describe("PATCH /users/[username]", function () {
  ...
  // TEST BUG #2
  test("should patch data if user being updated is curr user", async function () {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, first_name: "new-fn1" }); // u1 updating u1
    expect(response.statusCode).toBe(200);
    expect(response.body.user).toEqual({
      username: "u1",
      first_name: "new-fn1",
